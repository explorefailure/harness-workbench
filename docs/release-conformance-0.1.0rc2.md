...
`__all__` is read the same way: an assignment to that name anywhere in the
module's syntax counts, including one indented inside a conditional and
including `globals()["__all__"] = [...]`. The line-anchored text match this
replaces saw neither, so a routed-or-internal module could move its declared
surface with the routing and exported-name rules both skipping it.
