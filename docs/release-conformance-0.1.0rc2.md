...
| rc2 hosted preparation checkpoint | Exact public PR commit `f86e41031a4d6a98fbf3d0249d3a7c1416a5adc3` passed all eight Linux/macOS CPython 3.11–3.14 cells and the package job in [CI run 32604245910](https://github.com/explorefailure/harness-workbench/actions/runs/32604245910). It also passed [CodeQL run 32604245892](https://github.com/explorefailure/harness-workbench/actions/runs/32604245892); the repository had zero open code-scanning alerts when checked on 2026-08-22. These are preparation results for exactly that commit. Later audit-remediation commits require new hosted checks. |
| Audit-remediation lineage | Remediation begins at commit `05054d60c816309ea6346f2d34be1c654e7f5697` and includes later follow-up fixes. These commits follow hosted checkpoint `f86e41031a4d6a98fbf3d0249d3a7c1416a5adc3`; they require exact-head hosted checks after publication, and no checkpoint result is claimed for them until such a run is recorded. |
...
| `D-03` | Exact PR checkpoint `f86e41031a4d6a98fbf3d0249d3a7c1416a5adc3` passed the eight-cell matrix and package job in hosted CI run 32604245910. Audit-remediation changes supersede that checkpoint, and no release commit exists. | A recipient has hosted preparation evidence for one exact source commit, but no release-final evidence for the source eventually tagged | Successful required jobs for the final post-remediation commit and again for the exact release ref required by the release procedure |
...
| `D-07` | Exact PR checkpoint `f86e41031a4d6a98fbf3d0249d3a7c1416a5adc3` passed CodeQL run 32604245892, and the repository had zero open code-scanning alerts when checked on 2026-08-22. Later audit-remediation changes require a new CodeQL run; dated branch/tag ruleset evidence also remains pending. | A recipient can inspect the exact checkpoint result but cannot infer that later source passed or that required checks and immutable release refs are enforced | Successful CodeQL for the final post-remediation/release commit plus dated branch/tag ruleset evidence |
...
|---|---|---|---|
| Repository `unittest` suite | behavioral contracts plus documentation drift | local gate and `.github/workflows/ci.yml` matrix | Passed locally at `1a4713c`; exact PR checkpoint `f86e410` passed all eight hosted compatibility cells in CI run 32604245910. Post-audit source requires a new run. |
| Offline subject-adapter suite from source | shared-envelope producer, comparator, oracle, apparatus, and control contracts | every Linux/macOS and Python compatibility cell plus the maintainer source gate | Passed locally at `1a4713c` and in all eight hosted cells at `f86e410`; the release-engineering suite rejects conditional, forgiven, moved, excluded, commented, and unreachable forms of this gate. Post-audit source requires a new run. |
| Materialized subject-adapter suite from wheel and sdist | shipped subject tree and installed capture primitive agree | `tools/verify_installed_artifact.py`, separately for wheel and normalized sdist | Both locally built forms passed at `1a4713c`, and the package job passed for `f86e410`; downloaded release assets do not exist and remain pending. |
...
| Build plus strict Twine | package metadata/README rendering | local gate and package CI job | Passed locally at `1a4713c` and in the hosted package job at exact checkpoint `f86e410`; post-audit source and eventual release assets remain pending |
...
| Separate clean wheel and sdist install/first run | EF-SRS-03, `C-HWB-02`, `C-HWB-12` | `tools/verify_installed_artifact.py` | Passed locally at `1a4713c` and in the hosted package job at `f86e410`; downloaded release assets pending |
...
| Hosted matrix, CodeQL, repository settings, routes, tag and release | external release predicates | GitHub | Exact PR checkpoint `f86e410` passed CI run 32604245910 and CodeQL run 32604245892; zero open code-scanning alerts were observed on 2026-08-22. Post-audit exact-commit reruns, rulesets, tag, assets, and release remain **blocking**. |
