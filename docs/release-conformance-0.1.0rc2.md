...
Those were dropped before analysis until this commit, relative imports being
...
Two further spellings do reach the module namespace and are **not** read, on
the same grounds as the dynamic import above: `setattr(sys.modules[__name__],
"__all__", ...)`, and importing the name from elsewhere with
`from harness_workbench.capture import __all__`. Both were confirmed to bind —
the first governs `import *` — and both are disclosed scope rather than
coverage. Reading them would mean executing the module rather than parsing it,
and each closed spelling invites the next; the honest boundary is that this
rule reads assignments in the module's own syntax, not every route by which a
name can arrive.
