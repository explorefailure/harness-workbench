#!/usr/bin/env python3
"""Capture and judge one result-driven branch variant."""
...
import adapter
import branch_rewrite_oracle


HERE = Path(__file__).resolve().parent
INPUTS = (
    "run_branch_rewrite.sh",
    "branch_rewrite_runner.py",
    "branch_rewrite_oracle.py",
...
def main() -> int:
    ...
    try:
        capture = adapter.capture(
            HERE / "branch_rewrite_adapter_config.json",
            ...
            evidence_files=(
                ...
                },
                {
                    ...
                    "path": "branch-decision.jsonl",
                    "format": "jsonl",
                    "required": True,
                    "environment_variable": "PI_HWB_BRANCH_PATH",
                },
            ),
        )
        errors, comparison = branch_rewrite_oracle.evaluate(args.variant, capture)
    except (adapter.AdapterError, OSError, KeyError, TypeError) as error:
        print(json.dumps({
            "schema": "pi-hwb-branch-rewrite-run/v0.1",
            "error": str(error),
        }, sort_keys=True))
        return 2
    print(json.dumps({
